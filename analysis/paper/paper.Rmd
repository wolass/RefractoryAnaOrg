...
`r elicitorTab[1,9]`% of food elicited refractory cases had a previously confirmed diagnosis of food allergy. Severe cases of anaphylaxis were most frequently elicited by insects and food triggers.
Refractory anaphylaxis cases elicited by insects were more often elicited by bees than severe, non-refractory cases, p = `r table(data3$q_340_insects,data3$rANA) %>%  {rbind(addmargins(.[3:8,],1) %>% tail(1),.[2,])} %>% as.matrix() %>% fisher.test() %>% {.$p.value} %>% round(4)` (table \@ref(tab:elicitExact)). 
